...
function relativeTime(at?: number) {
  if (!at) return t("webhooks.time.never");
...
function deliverySummary(run: RoutineRun) {
  const eventName = run.prompt?.match(/^Event: (.+)$/m)?.[1]?.trim() || t("webhooks.event");
...
function suggestedName(prompt: string, bot?: Bot) {
  ...
  return first || t("webhooks.fallbackName", { name: bot?.name ?? "MAUS" });
}

function statusFor(webhook: WebhookTrigger) {
  if (webhook.verificationPending) return { label: t("webhooks.status.waiting"), tone: "text-accent", dot: "bg-accent animate-pulse" };
  if (webhook.verifiedAt && !webhook.enabled) return { label: t("webhooks.status.readyToEnable"), tone: "text-warning", dot: "bg-warning" };
  if (webhook.enabled) return { label: t("webhooks.status.active"), tone: "text-success", dot: "bg-success" };
  return { label: t("webhooks.status.paused"), tone: "text-ink-secondary", dot: "bg-ink-secondary/50" };
}
...
function runStatusLabel(status: RoutineRun["status"]): string {
  if (status === "waiting") return t("webhooks.outcome.needsYou");
  if (status === "queued") return t("webhooks.outcome.queued");
  if (status === "running") return t("webhooks.outcome.running");
  if (status === "completed") return t("webhooks.outcome.completed");
  if (status === "failed") return t("webhooks.outcome.failed");
  if (status === "missed") return t("webhooks.outcome.missed");
  return status[0]!.toUpperCase() + status.slice(1);
}

function outcomeLabel(outcome: WebhookAttempt["outcome"], run?: RoutineRun) {
  if (run) return runStatusLabel(run.status);
  if (outcome === "captured") return t("webhooks.outcome.testReceived");
  if (outcome === "duplicate") return t("webhooks.outcome.duplicate");
  if (outcome === "ignored") return t("webhooks.outcome.ignored");
  if (outcome === "rejected") return t("webhooks.outcome.rejected");
  return t("webhooks.outcome.accepted");
}
...
function WebhookEditor({ webhook, bots, onClose, onCredential }: { webhook?: WebhookTrigger; bots: Bot[]; onClose: () => void; onCredential: (credential: WebhookCredential, webhookId: string) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-5 backdrop-blur-sm" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-label={webhook ? t("webhooks.editor.edit") : t("webhooks.editor.new")} tabIndex={-1} className="flex max-h-[90vh] w-full max-w-[590px] flex-col overflow-hidden rounded-2xl border border-hairline/60 bg-panel shadow-2xl">
        <div className="flex items-start justify-between border-b border-hairline/40 px-5 py-4"><div><div className="text-[17px] font-semibold text-ink">{webhook ? t("webhooks.editor.edit") : t("webhooks.editor.new")}</div><div className="mt-1 text-[12px] text-ink-secondary">{t("webhooks.editor.subtitle")}</div></div><button onClick={onClose} aria-label={t("webhooks.editor.closeAria")} className="rounded-lg p-2 text-ink-secondary hover:bg-raised hover:text-ink"><X size={18} /></button></div>
        <div className="min-h-0 flex-1 space-y-5 overflow-y-auto p-5">
          <div><div className="mb-2 text-[12px] font-medium text-ink-secondary">{t("webhooks.editor.who")}</div><div className="grid grid-cols-2 gap-2 sm:grid-cols-3">{bots.map((bot) => <button key={bot.id} type="button" data-initial-focus={botId === bot.id ? "" : undefined} onClick={() => setBotId(bot.id)} className={cn("flex min-w-0 items-center gap-2 rounded-xl border px-3 py-2 text-left", botId === bot.id ? "border-accent/70 bg-accent/10" : "border-hairline/50 bg-inset hover:bg-raised/60")}><BotAvatar bot={bot} state={botId === bot.id ? "happy" : "idle"} size={38} animated={false} /><span className="min-w-0 flex-1 truncate text-[13px] font-medium text-ink">{bot.name}</span></button>)}</div></div>
          <div className="rounded-xl border border-accent/20 bg-accent/5 px-3.5 py-3 text-[11.5px] leading-relaxed text-ink-secondary">{t("webhooks.editor.payloadBefore")} <code className="text-ink">{`{"task":"Check the failed build"}`}</code>{t("webhooks.editor.payloadAfter")}</div>
          <details className="group rounded-xl border border-hairline/45 bg-inset/45 px-4 py-3" open={Boolean(webhook)}>
            <summary className="cursor-pointer text-[12.5px] font-medium text-ink">{t("webhooks.editor.advanced")}</summary>
            <div className="mt-4 space-y-4">
              <label className="block"><span className="mb-1.5 block text-[11.5px] font-medium text-ink-secondary">{t("webhooks.editor.name")} <span className="font-normal">{t("webhooks.editor.optional")}</span></span><input value={name} onChange={(event) => setName(event.target.value)} placeholder={suggestedName(prompt, bots.find((bot) => bot.id === botId))} className="w-full rounded-xl border border-hairline/60 bg-panel px-3.5 py-2.5 text-[13px] text-ink outline-none placeholder:text-ink-secondary/60 focus:border-accent/70" /></label>
              <label className="block"><span className="mb-1.5 block text-[11.5px] font-medium text-ink-secondary">{t("webhooks.editor.instructions")} <span className="font-normal">{t("webhooks.editor.optional")}</span></span><textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} rows={3} placeholder={t("webhooks.editor.instructionsPlaceholder")} className="w-full resize-y rounded-xl border border-hairline/60 bg-panel px-3.5 py-3 text-[13px] leading-relaxed text-ink outline-none placeholder:text-ink-secondary/60 focus:border-accent/70" /><span className="mt-1.5 block text-[10.5px] leading-relaxed text-ink-secondary">{t("webhooks.editor.instructionsHint")}</span></label>
              <div><div className="mb-2 text-[11.5px] font-medium text-ink-secondary">{t("webhooks.editor.runOn")}</div><div className="grid grid-cols-2 gap-2"><button type="button" onClick={() => setRunOn("maus")} className={cn("rounded-xl border p-3 text-left", runOn === "maus" ? "border-accent/70 bg-accent/10" : "border-hairline/50 bg-panel hover:bg-raised/60")}><div className="flex items-center gap-2 text-[12.5px] font-medium text-ink"><Laptop size={14} />{t("webhooks.thisComputer")}</div></button><button type="button" disabled={!cloudReady && runOn !== "cloud"} onClick={() => setRunOn("cloud")} className={cn("rounded-xl border p-3 text-left disabled:cursor-not-allowed disabled:opacity-45", runOn === "cloud" ? "border-accent/70 bg-accent/10" : "border-hairline/50 bg-panel hover:bg-raised/60")}><div className="flex items-center gap-2 text-[12.5px] font-medium text-ink"><Cloud size={14} />{t("webhooks.cloudVm")}</div></button></div></div>
              <label className="block"><span className="mb-1.5 block text-[11.5px] font-medium text-ink-secondary">{t("webhooks.editor.eventTypes")} <span className="font-normal">{t("webhooks.editor.optional")}</span></span><input value={eventTypes} onChange={(event) => setEventTypes(event.target.value)} placeholder="push, workflow_run" className="w-full rounded-xl border border-hairline/60 bg-panel px-3.5 py-2.5 text-[13px] text-ink outline-none placeholder:text-ink-secondary/60 focus:border-accent/70" /><span className="mt-1.5 block text-[10.5px] text-ink-secondary">{t("webhooks.editor.eventTypesHint")}</span></label>
            </div>
          </details>
          {error && <div className="rounded-xl border border-danger/30 bg-danger/10 px-3.5 py-3 text-[12px] text-danger">{error}</div>}
        </div>
        <div className="flex justify-end gap-2 border-t border-hairline/40 px-5 py-4"><button onClick={onClose} className="rounded-xl px-4 py-2 text-[13px] text-ink-secondary hover:bg-raised hover:text-ink">{t("webhooks.editor.cancel")}</button><button disabled={saving || !botId} onClick={() => void save()} className="flex items-center gap-2 rounded-xl bg-accent px-4 py-2 text-[13px] font-medium text-white hover:brightness-110 disabled:opacity-40">{saving && <Loader2 size={14} className="animate-spin" />}{webhook ? t("webhooks.editor.save") : t("webhooks.editor.create")}</button></div>
      </div>
    </div>
  );
}
...
export function WebhooksPanel({
  ...
  const activity = useMemo<ActivityItem[]>(() => {
    ...
    const attempts = state.webhookAttempts
      ...
      .map((attempt) => ({
        ...
        eventName: attempt.eventName || (attempt.outcome === "rejected" ? t("webhooks.rejectedRequest") : t("webhooks.event")),
  ...
  const invoke = async (webhook: WebhookTrigger, action: "toggle" | "delete") => {
    ...
    try {
      if (action === "delete") {
        ...
        if (enabling && webhook.verificationPending) throw new Error(t("webhooks.testFirst"));
  ...
  const createAndCopyCommand = async (webhook: WebhookTrigger, replace = false) => {
    if (replace && !window.confirm(t("webhooks.rotateConfirm"))) return;
      ...
      <div className="mx-auto max-w-[1120px] space-y-4">
        ...
        {state.webhooks.length === 0 ? (
          <div className="border-t border-hairline/40 px-6 py-16 text-center">
            ...
            <h3 className="text-[16px] font-semibold text-ink">{t("webhooks.empty.title")}</h3>
            <p className="mx-auto mt-2 max-w-[420px] text-[12.5px] leading-relaxed text-ink-secondary">{t("webhooks.empty.subtitle")}</p>
            <button onClick={() => setEditor("new")} disabled={bots.length === 0} className="mt-5 inline-flex items-center gap-2 rounded-xl bg-accent px-4 py-2.5 text-[13px] font-medium text-white hover:brightness-110 disabled:opacity-40"><Plus size={15} />{t("webhooks.editor.create")}</button>
            {bots.length === 0 && <p className="mt-3 text-[12px] text-warning">{t("webhooks.empty.needBot")}</p>}
          </div>
        ) : selected && (
          <div className="min-h-[580px] overflow-hidden rounded-2xl border border-hairline/45 bg-panel/25 md:grid md:grid-cols-[250px_minmax(0,1fr)]">
            <aside className="border-b border-hairline/40 bg-inset/35 p-2.5 md:border-r md:border-b-0">
              <div className="flex items-center justify-between px-2 py-2">
                <span className="text-[10px] font-medium uppercase tracking-[0.12em] text-ink-secondary">{t("webhooks.yours")}</span>
              ...
              <div className="flex flex-wrap items-start justify-between gap-3 px-5 pt-5 md:px-7 md:pt-6">
                <div className="flex min-w-0 items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="mt-1 flex flex-wrap items-center gap-1.5 text-[10.5px] text-ink-secondary"><span>{selectedBot?.name ?? t("webhooks.deletedBot")}</span><span>·</span><span>{selected.runOn === "cloud" ? t("webhooks.cloudVm") : t("webhooks.thisComputer")}</span></div>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  {!selected.verificationPending && <button disabled={Boolean(working)} onClick={() => void invoke(selected, "toggle")} className={cn("flex items-center gap-1.5 rounded-lg px-3 py-2 text-[11.5px] font-medium", selected.enabled ? "bg-raised text-ink-secondary hover:text-ink" : "bg-accent text-white hover:brightness-110")}>{selected.enabled ? <Pause size={13} /> : <Play size={13} />}{selected.enabled ? t("webhooks.pause") : t("webhooks.enable")}</button>}
                ...
                <div className="px-5 py-6 md:px-7">
                  <div className="max-w-[720px]">
                    ...
                    {credential ? (
                      ...
                      <div className="mt-4">
                        <p className="max-w-[560px] text-[10.5px] leading-relaxed text-ink-secondary">{t("webhooks.urlOnce")}</p>
                    ...
                    <div className="mt-3 flex items-start gap-2 text-[10.5px] leading-relaxed text-ink-secondary"><Laptop size={12} className="mt-0.5 shrink-0" /><span>{t("webhooks.keepOpen")}</span></div>
                    ...
                    <div className="mt-7 border-t border-hairline/35 pt-5">
                      <div className="grid gap-4 text-[11.5px] sm:grid-cols-2"><div><div className="text-[10px] uppercase tracking-wider text-ink-secondary">{t("webhooks.tasksGoTo")}</div><div className="mt-1.5 font-medium text-ink">{selectedBot?.name ?? t("webhooks.deletedBot")}</div></div><div><div className="text-[10px] uppercase tracking-wider text-ink-secondary">{t("webhooks.runsOn")}</div><div className="mt-1.5 font-medium text-ink">{selected.runOn === "cloud" ? t("webhooks.cloudVm") : t("webhooks.thisComputer")}</div></div></div>
                    </div>

                    <details className="group mt-5 border-t border-hairline/35 pt-4"><summary className="flex cursor-pointer list-none items-center justify-between text-[11.5px] font-medium text-ink"><span>{t("webhooks.advanced")}</span><ChevronDown size={14} className="text-ink-secondary transition-transform group-open:rotate-180" /></summary><div className="mt-4 space-y-3 text-[10.5px] leading-relaxed text-ink-secondary">{selected.prompt ? <p><span className="font-medium text-ink">{t("webhooks.defaultInstruction")}</span> {selected.prompt}</p> : <p>{t("webhooks.noDefaultInstruction")}</p>}{selected.eventTypes?.length ? <p><span className="font-medium text-ink">{t("webhooks.acceptedEvents")}</span> {selected.eventTypes.join(", ")}</p> : <p>{t("webhooks.allEvents")}</p>}<button onClick={() => setEditor(selected)} className="rounded-lg border border-hairline/50 px-3 py-2 text-[11px] font-medium text-ink hover:bg-raised">{t("webhooks.editSettings")}</button></div></details>
                  </div>
                </div>
              ) : (
                <div className="px-5 py-5 md:px-7">
                  <div className="mb-3"><h4 className="text-[13px] font-semibold text-ink">{t("webhooks.recentDeliveries")}</h4><p className="mt-1 text-[10.5px] text-ink-secondary">{t("webhooks.recentHint")}</p></div>
